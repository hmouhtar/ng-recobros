...
export class RolesService {
  ...
  async getEditableRoles(): Promise<string[]> {
    const rolesKeys = (await this.getAllRoles()).map((role) => role.rolName);
    const asyncFiltered = await Promise.all(
