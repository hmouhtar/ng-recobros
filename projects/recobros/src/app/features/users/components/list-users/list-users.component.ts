...
export class ListUsersComponent implements OnInit {
  ...
  ngOnInit(): void {
    Promise.all([this.userService.getCurrentUser(), this.userService.getUsers()]).then((res) => {
      let [currentUser, allUsers] = res;
      if (["RECOVERY_ADMINISTRATOR", "COMPANY_RECOVERY_MANAGER "].includes(currentUser.rol)) {
  ...
  deleteUser(username) {
    if (confirm("Procederás a eliminar al usuario. Esta acción no es reversible. ¿Estás seguro?")) {
